...
save = {
    ...
    set = function(self)
        ...
        if data then
            ...
            local money,model,dbid = source:getData('money'), source.model, source:getData('dbid')

            local query = self.mysql:exec("UPDATE `characters` SET `money`='"..(money).."', `model`='"..(model).."', `pos`='"..(pos).."'  WHERE `id`='"..(dbid).."'")
